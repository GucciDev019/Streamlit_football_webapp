...
def pass_network(df_half):
  ...
  avg_loc_df = split_coordinates_pass(avg_loc_df)
...
##CARRIES
def dfcarries_preparation(df_input, team_name):
  #selecting only carries
  df = df_input.copy()
  df = df[(df['team'] == team_name) & (df['type'] == 'Carry')]
  df = df.reset_index(drop=True)
  ...
  #splitting coordinates
  for i in range(len(df)):
    df.loc[i,'carry_end_location'] = df.loc[i,'carry_end_location'][1:-1]
  df[['end_X','end_Y']] = df.carry_end_location.str.split(",",expand=True)
  #converting splitted coordinates to float
  for column in ('end_X', 'end_Y'):
    for i in range(len(df)):
      df.loc[i, column] = float(df.loc[i, column])
...
def carry_length(x, y, endx, endy):
  return math.sqrt(pow(endx - x, 2) + pow(endy - y, 2))

def zones_carries(df, zone1, zone2, pitch, axs, dist):
  for i in range(len(df)):
    x_values = [df.loc[i, 'X'], df.loc[i, 'end_X']]
    y_values = [df.loc[i, 'Y'], df.loc[i, 'end_Y']]

    if (df.loc[i, 'X'] <= zone1) & (carry_length(x_values[0], y_values[0], x_values[1], y_values[1]) >= dist):
      pitch.plot(x_values, y_values, color='orange', ax=axs[0])
      pitch.scatter(df.loc[i, 'end_X'],df.loc[i, 'end_Y'],color='orange', ax = axs[0])
    elif (df.loc[i, 'X'] > zone1) & (df.loc[i, 'X'] <= zone2) & (carry_length(x_values[0], y_values[0], x_values[1], y_values[1]) >= dist):
      pitch.plot(x_values, y_values, color='orange', ax=axs[1])
      pitch.scatter(df.loc[i, 'end_X'],df.loc[i, 'end_Y'],color='orange', ax = axs[1])
    elif (df.loc[i, 'X'] > zone2) & (carry_length(x_values[0], y_values[0], x_values[1], y_values[1]) >= dist):
      pitch.plot(x_values, y_values, color='orange', ax=axs[2])
      pitch.scatter(df.loc[i, 'end_X'],df.loc[i, 'end_Y'],color='orange', ax = axs[2])

def zones_carriesmap(df_input, team_name, dist):

  df = dfcarries_preparation(df_input, team_name)
  ...
  pitch.draw(ax=axs[0])
  axs[0].set_title("Defensive carries", color="white", fontsize=16)
  pitch.draw(ax=axs[1])
  axs[1].set_title("Midfield carries", color="white", fontsize=16)
  pitch.draw(ax=axs[2])
  axs[2].set_title("Offensive carries", color="white", fontsize=16)

  zones_carries(df, 25, 75, pitch, axs, dist)
  return fig
...
def plot_action(df_in, team_name, actions, pitch, ax, colors, markers):
  df_in = df_in[df_in['team'] == team_name]
  #select the action to plot
  for action, marker, col in zip(actions, markers, colors):
    df = df_in[df_in['type'] == action].copy()
    df = df.reset_index(drop=True)
    #coordinates
    df = split_coordinates_act(df)
    #plot
    for i in range(len(df) - 1):
      pitch.scatter(df.loc[i, 'X'],df.loc[i, 'Y'], s = 200, marker=marker, c=col, ax = ax)

    #trick to plot the  legend
    pitch.scatter(df.loc[len(df) - 1, 'X'],df.loc[len(df) - 1, 'Y'], s = 200, marker=marker, c=col, ax = ax, label=action)
    ax.legend(loc = "upper left", fontsize = 13)

def plot_action_graph(df):
    plt.rcParams["figure.figsize"] = (25,20)
    pitch = Pitch(pitch_color='black', line_color='white')
    fig, ax = plt.subplots()
    fig.patch.set_facecolor('black')
    ...
    plot_action(df, 'Houston Dash', ['Ball Recovery', 'Miscontrol', 'Foul Won'], pitch, ax, ['orange', 'red', 'green'], ['o','*', 'o'])
    return fig
...
###DASHBOARD SCHEMA
st.title("Houston Dash - OL Reign")
st.markdown("""General statistics displayed, explore other statistics clicking the buttons""")

##OPTIONS
st.sidebar.header('Analysis')
analysis = ['Match', 'Players']
selected_analysis = st.sidebar.selectbox('Select', analysis)

if(selected_analysis == 'Match'):

    st.sidebar.header('Options')
    match_analysis = ['General', 'XGoals', 'Passing', 'Passing - in depth', 'Carries', 'Pressure', 'General actions']
    selected_match_analysis = st.sidebar.selectbox('select the analysis to display', match_analysis)

    if selected_match_analysis == 'General':

        # FIRST PAGE - GENERAL STATISTICS
        st.metric(label="Houston Dash", value=2)
    ...
    elif selected_match_analysis == 'Carries':

        length = st.select_slider('carry length', options=list(range(100)), value=15)
        st.pyplot(zones_carriesmap(df, "Houston Dash", length))

    elif selected_match_analysis == 'Pressure':
        st.pyplot(plot_eventhm(df, "Houston Dash", "Pressure"))
    #elif selected_match_analysis == 'General Actions':
